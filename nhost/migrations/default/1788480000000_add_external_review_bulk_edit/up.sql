...
-- Dropped explicitly because Postgres rejects a return-type change via CREATE OR REPLACE.
-- Earlier revisions of this migration returned jsonb.
DROP FUNCTION IF EXISTS public.apply_external_review_edit_batch(uuid, integer, text);

-- Returns SETOF the batch table (not jsonb): Hasura can only track functions whose
-- return type is a tracked table, so a scalar return cannot be exposed on mutation_root.
CREATE OR REPLACE FUNCTION public.apply_external_review_edit_batch(
  p_batch_id uuid,
  p_confirm_changed_count integer,
  p_admin_user_id text
)
RETURNS SETOF public.external_review_edit_batches
LANGUAGE plpgsql
AS $$
DECLARE
