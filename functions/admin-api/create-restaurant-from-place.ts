...
import { createRestaurantFromPlace } from '../_lib/admin-restaurant-handlers'
...
export default withAdminApiAuth(async (req: Request, res: Response) => {
  ...
  try {
    const result = await createRestaurantFromPlace(readJsonBody(req))
    adminSuccess(res, result.data, undefined, 201)
  } catch (error) {
    adminError(
      res,
      error instanceof Error ? error.message : 'Failed to create restaurant from place',
      400,
    )
  }
})
