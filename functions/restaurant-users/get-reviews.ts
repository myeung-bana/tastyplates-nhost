...
export default async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = status
      ? await hasuraQuery<Result>(GET_USER_REVIEWS_BY_STATUS, { authorId: userId, status, limit, offset })
      : canReadPrivate
        ? await hasuraQuery<Result>(GET_ALL_USER_REVIEWS, { authorId: userId, limit, offset })
        : await hasuraQuery<Result>(GET_PUBLIC_USER_REVIEWS, { authorId: userId, limit, offset })

    if (result.errors?.length) {
      console.error('[restaurant-users/get-reviews]', result.errors)
