...
export default async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const followsResult = await hasuraQuery<FollowsResult>(GET_FOLLOWERS_LIST, { userId, limit, offset })
    ...
    const usersResult = await hasuraQuery<UsersResult>(GET_USERS_BY_IDS, { ids: followerIds })

    if (usersResult.errors?.length) {
      console.error('[restaurant-users/get-followers-list]', usersResult.errors)
